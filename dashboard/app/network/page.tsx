...
const COLORS = ["#34d399", "#8b5cf6", "#3abff8", "#fbbf24", "#f97316"];
...
export default function NetworkPage() {
  ...
  const [peers, setPeers] = useState<PeerNode[]>([
    { id: "gw", label: "You", x: 50, y: 50, size: 40, color: "#3abff8", type: "gateway" },
  ]);
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* Left Panel */}
      <aside className="w-96 border-r border-slate-800 flex flex-col overflow-y-auto flex-shrink-0">
        {/* Header */}
        <div className="px-5 py-4 border-b border-slate-800">
          <div className="flex items-center gap-2 text-xs text-slate-400 mb-3">
            <span>Dashboard</span>
            <span className="material-symbols-outlined text-sm">chevron_right</span>
            <span className="text-white">Gossip Status</span>
          </div>
          <div className="flex items-center justify-between">
            <h1 className="text-lg font-bold">Network Status</h1>
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-[10px]">
        ...
        <div className="p-5 space-y-3">
          <div className="bg-[#1b2427] rounded-xl border border-slate-800 p-4">
            <div className="flex items-center gap-3 mb-2">
              <span className="material-symbols-outlined text-[#3abff8]">chat_bubble</span>
              <div>
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Gossip Messages</p>
          ...
          <div className="bg-[#1b2427] rounded-xl border border-slate-800 p-4">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Unique Txns Synced</p>
          ...
          <div className="bg-[#1b2427] rounded-xl border border-slate-800 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Active Peers</p>
        ...
        <div className="px-5 pb-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="text-[10px] text-[#3abff8] hover:underline">View All</button>
          </div>
            ...
            <div className="space-y-4">
              {logEntries.map((log, i) => (
                <div key={i} className="relative flex items-start gap-3">
                  <div
                    className="w-3.5 h-3.5 rounded-full border-2 border-[#111618] flex-shrink-0 -ml-[7.5px] z-10"
                    style={{ backgroundColor: log.color }}
                  />
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono text-[#3abff8]">{log.hash}</span>
              ...
              {logEntries.length === 0 && (
                <p className="text-[10px] text-slate-500">No reconstructed gossip entries yet.</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="px-5 pb-5 mt-auto">
          <div className="rounded-xl border border-slate-800 bg-[#1b2427] p-4 flex items-start gap-3">
            <span className="material-symbols-outlined text-[#3abff8] text-lg">offline_bolt</span>
            <div>
              ...
              <button className="text-[10px] text-[#3abff8] mt-2 hover:underline flex items-center gap-1">
                Read Documentation
      ...
      <div className="flex-1 relative bg-[#0f172a] overflow-hidden">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "linear-gradient(rgba(58,191,248,0.3) 1px, transparent 1px), linear-gradient(90deg, rgba(58,191,248,0.3) 1px, transparent 1px)",
        ...
        <div className="relative z-10 p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-[#1b2427] border border-slate-800 text-xs text-slate-300 hover:border-[#3abff8]/40 transition"
            >
              Broadcast Gossip
            </button>
            <label className="flex items-center gap-2 cursor-pointer">
              <span className="text-[10px] text-slate-400">Show Offline</span>
              <button
                onClick={() => setShowOffline(!showOffline)}
                className={`w-8 h-4 rounded-full transition-all flex items-center ${
                  showOffline ? "bg-[#3abff8] justify-end" : "bg-slate-700 justify-start"
                }`}
              >
        ...
        {gossipStatus && (
          <p className="relative z-10 px-4 text-[10px] text-slate-400">{gossipStatus}</p>
        )}
          ...
          <svg viewBox="0 0 100 100" className="w-[600px] h-[600px]">
            ...
            {filteredPeers.map((peer) => (
              <g key={peer.id}>
                ...
                {peer.type === "gateway" && (
                  <>
                    <circle cx={peer.x} cy={peer.y} r="6" fill="none" stroke="#3abff8" strokeWidth="0.3" opacity="0.2">
                ...
                <text
                  ...
                  textAnchor="middle"
                  fontSize="2"
                  fill={peer.type === "offline" ? "#475569" : "#94a3b8"}
                >
                  {peer.label}
                </text>
                {peer.type === "offline" && (
                  <text x={peer.x} y={peer.y - peer.size / 10 - 1.5} textAnchor="middle" fontSize="1.5" fill="#475569">
                    OFFLINE
                  </text>
                )}
              </g>
        ...
        {/* Legend */}
        <div className="absolute bottom-4 left-4 bg-[#1b2427]/80 backdrop-blur-md rounded-xl border border-slate-800 p-3 z-10">
          <p className="text-[10px] font-semibold text-slate-300 mb-2">Network Topology</p>
          <div className="space-y-1.5">
            {[
              { color: "#8b5cf6", label: "High Bandwidth" },
              { color: "#34d399", label: "Standard" },
              { color: "#475569", label: "Offline" },
              { color: "#3abff8", label: "Active Stream" },
            ].map((item) => (
        ...
        <div className="absolute bottom-4 right-4 flex flex-col gap-1 z-10">
          {[
            ...
            <button
              key={btn.icon}
              title={btn.label}
              className="w-8 h-8 rounded-lg bg-[#1b2427] border border-slate-800 flex items-center justify-center text-slate-400 hover:text-white hover:border-[#3abff8]/30 transition"
            >
